...
class model_diffusion(torch.nn.Module):
  def __init__(self,input):
    ...
    self.linear_x = nn.Linear(int(input*0.25),input)
    self.linear_y = nn.Linear(int(input*0.25),input)
    self.linear_t = nn.Linear(int(input*0.25),input)
  def forward(self,x,y,t):
    t = self.act(self.linear(torch.ones_like(x)*t))
    x = self.act(self.linear(x))
#    print(x.shape)
    y = self.act(self.linear(y))
#    print((torch.ones_like(x)*t).shape)
    
    return self.linear_x(x) + self.linear_y(y) + self.linear_t(t)